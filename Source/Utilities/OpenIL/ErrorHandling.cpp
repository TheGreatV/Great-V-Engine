...
GreatVEngine::OpenIL::Exception::Exception(const ErrorCode& errorCode_):
	errorText(GetErrorText(errorCode_))
{
}
GreatVEngine::OpenIL::Exception::Exception(const ErrorText& errorText_) :
	errorText(errorText_)
{
}
GreatVEngine::OpenIL::ErrorText GreatVEngine::OpenIL::Exception::GetText() const
{
	return errorText;
}


bool GreatVEngine::OpenIL::IsSuccessful(const ErrorCode& errorCode)
{
	return errorCode == IL_NO_ERROR;
}
GreatVEngine::OpenIL::ErrorText GreatVEngine::OpenIL::GetErrorText(const ErrorCode& errorCode)
{
	switch(errorCode)
	{
...
void GreatVEngine::OpenIL::ErrorTest()
{
	auto errorCode = ilGetError();

	ErrorTest(errorCode);
}
void GreatVEngine::OpenIL::ErrorTest(const ErrorCode& errorCode)
{
