...
uniform	vec3	environmentRangeNear;
uniform	vec3	environmentRangeFar;
uniform	vec3	environmentRangeA;
uniform	vec3	environmentRangeB;
...
void main()
{
	...
	vec3	offsetInverse = 1.0f - clamp((abs(offset) - environmentRangeNear) / (environmentRangeFar - environmentRangeNear), vec3(0.0f), vec3(1.0f));
	vec3	rangeIntensity = environmentRangeA * offsetInverse * offsetInverse + environmentRangeB * offsetInverse;
	float	totalRangeIntensity = cos((1.0f - offsetInverse.x) * PI * 0.5f) * cos((1.0f - offsetInverse.y) * PI * 0.5f) * cos((1.0f - offsetInverse.z) * PI * 0.5f);
	...
	oSpecular = vec4(totalRangeIntensity*diffuseIntensity*specularIntensity * environment*environmentColor*specular, totalRangeIntensity);
}
